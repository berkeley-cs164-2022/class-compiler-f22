...
let print_x_first_version () = 
    print_endline ("what is our first x? is it *still* 1? \t" ^  (string_of_int  x))

let x = x + 1
let print_x_second_version () = 
    print_endline ("what is our second x? \t\t\t" ^ string_of_int  x)
...
let show_unrolled () =
    let x = 1 in
        let print_x_first_version () = 
            print_endline ("what is our first x? is it *still* 1? \t" ^  (string_of_int  x)) in
            (* what scope are we in here?  so what's x on the line below? *)
            let x = x + 1 in 
                (* what scope are we in here?  so what's x on the line below this? *)
                let print_x_second_version () = 
                    print_endline ("what is our second x? \t\t\t" ^ string_of_int  x) in
